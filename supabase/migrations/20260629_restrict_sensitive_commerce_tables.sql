-- Restrict sensitive commerce, observability and AI tables after the audit.
-- Strategy: additive/non-destructive only. This migration creates missing tables with
-- restrictive RLS, tight grants and owner/admin policies. It intentionally does not
-- drop or rewrite existing transactional data.
-- Rollback plan: if an application flow is unexpectedly blocked, temporarily revoke
-- the affected policy change with a targeted follow-up migration and keep service_role
-- access available for reconciliation/backfill jobs; do not drop these tables in prod.
...
CREATE INDEX IF NOT EXISTS payments_provider_payment_id_idx ON public.payments(provider_payment_id) WHERE provider_payment_id IS NOT NULL;
...
DROP POLICY IF EXISTS "events_insert_own_or_anonymous" ON public.events;
...
CREATE POLICY "events_insert_own_or_anonymous"
  ON public.events
  FOR INSERT
  TO authenticated
